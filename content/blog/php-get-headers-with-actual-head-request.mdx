...
PHP has a function called [`get_headers`](http://php.net/manual/en/function.get-headers.php) which, as you'd guess, gives you the headers returned from an HTTP request. _However_ it actually uses a `GET`, rather than a `HEAD`, request.
