...
While trying to figure out of [PayPal Payment Data Transfers](/?p=1210) I came over an issue that caused my [cURL](http://php.net/manual/en/book.curl.php) requests to fail.

These were the [cURL options](http://php.net/manual/en/function.curl-setopt.php) I used:
...
1. Go to [curl.haxx.se/docs/caextract.html](http://curl.haxx.se/docs/caextract.html)
2. Download the [cacert.pem](http://curl.haxx.se/ca/cacert.pem) and put it somwhere, for example `C:\wamp\cacert.pem`
...
1. Go to [curl.haxx.se/docs/caextract.html](http://curl.haxx.se/docs/caextract.html)
2. Download the [cacert.pem](http://curl.haxx.se/ca/cacert.pem) and put it for example next to your script.
...
Since we are using the [HTTPS](http://en.wikipedia.org/wiki/Https) protocol we need to somehow verify the one we are talking to. Your browser does this by checking their certificate against several built-in certificate stores from trusted certificate suppliers, like for example VeriSign.
