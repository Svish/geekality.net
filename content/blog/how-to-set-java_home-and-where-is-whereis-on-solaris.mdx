...
Well, turns out you can! Apparently the command actually <em>does</em> exist. It's just not in your path by default. This command (and some others) can be found in `/usr/ucb`, so we just need to add that to our `PATH` and we can use it like normal.
