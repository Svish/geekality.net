...
Just stumbled over a handy function to make that number of bytes a bit more readable for us humans. I found it in <a href="http://php.net/manual/en/function.disk-total-space.php">a comment in the PHP manual</a> and figured I could note it down here so I don't lose it and in case someone else could need it. A function like that is just bound to come in handy some day...

Let me know if you have a better name for it, but here it is 🙂

<ins dateTime="2013-04-11T21:10:45+00:00">
  **2013-04-11** Added zero check (prevents crash if `$bytes` is zero) and abs
  call (prevents crash when `$bytes` is negative)
</ins>
