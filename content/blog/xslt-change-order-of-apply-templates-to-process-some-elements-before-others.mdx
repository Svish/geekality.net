...
The order of the elements is the problem here since the input order is random. The `Value` element won't be the first in the output, and the whole transformation will actually crash because it'll try to add attributes to the `Option` node <em>after</em> child nodes has been added, which is not allowed.
