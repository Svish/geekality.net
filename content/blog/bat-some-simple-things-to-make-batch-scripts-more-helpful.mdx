...
```dos
  REM Some comment REM Will still appear as a command if you haven't disabled
  echoing
```
...
```dos
goto end REM Will be skipped :end
```
...
```dos
if "%~1"=="" (echo Usage: test.bat path && goto end) if not exist %~1 (echo
%~1 does not exist && goto end)
```

## Simpe choices

```dos
choice /c ny /n /d y /t 10 /m "Do you really want to do this? [y=10s,n] if
ERRORLEVEL 2 ( echo Ok, I'll do it )
```
