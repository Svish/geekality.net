...
I'm using a simple PHP <a href="https://github.com/softius/php-cross-domain-proxy">cross-domain-proxy</a> to be able to do some Javascript requests towards an API on a different domain. Worked great, until I needed to do basic authentication. I set the appropriate header to be passed through, `'Authorization': 'Basic ' + btoa(username+':'+password)`, but in the proxy script, that header had vanished.
