...
What we are looking for here is something called the [Lowest Common Multiple](http://en.wikipedia.org/wiki/Least_common_multiple) (LCM). The LCM of all the divisors, 1 to 20, is the answer to this problem. To calculate the LCM of all those numbers we first need two methods.

The first is one that finds the LCM of two numbers. The formula for that, however, is based on another method which finds the [Greatest Common Divisor](http://en.wikipedia.org/wiki/Greatest_common_divisor) (GCD) of two numbers. In other words we need to make that method first.

I implemented the GCD method using the [Euclidean algorithm](http://en.wikipedia.org/wiki/Euclidean_algorithm), and it goes like this:
