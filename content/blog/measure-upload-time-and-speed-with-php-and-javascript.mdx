...
Stumbled upon [a question on StackOverflow](http://stackoverflow.com/q/8181291/39321) the other day which got me curious. The question was about how to measure how long it takes to upload a file to a PHP script. This is what had been tried out:
...
And then we need the Javascript which does the AJAX request (using [jQuery](http://api.jquery.com/jQuery.ajax/)). Notice that this makes sure we have gotten a response from the timer script before we actually submit the form.
...
And that's pretty much all there is to it! Working sample at [samples.geekality.net/upload-timer](http://samples.geekality.net/upload-timer/).
