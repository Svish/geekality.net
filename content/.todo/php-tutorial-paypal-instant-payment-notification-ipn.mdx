...
<i>
  ...
  the SSL verification. This happened to me on one of my hosts, so [documented
  it here](http://www.geekality.net/?p=1413).
</i>
...
I have put together a working sample you can check out over at [samples.geekality.net/ipn](http://samples.geekality.net/ipn/). Hopefully this tutorial and that sample can help you get started with all of this 🙂

The sample uses an [IPN handler class](http://samples.geekality.net/view-source.php?file=ipn/ipn_handler.class.php) I put together, that should be usable pretty much out of the box. Just extend the class, override the `process` method and do what you need. Below is the [simple sample](http://samples.geekality.net/view-source.php?file=ipn/listener.php) which just appends the incoming messages to a file.
